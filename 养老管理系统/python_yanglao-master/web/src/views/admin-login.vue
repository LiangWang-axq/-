...
const handleLogin = () => {
  userStore.adminLogin({
    ...
    // 根据角色跳转到对应页面
    const userRole = userStore.admin_user_role || localStorage.getItem('ADMIN_USER_ROLE') || '0';
    ...
    switch (userRole) {
      case '0': // 管理员
        ...
        break;
      case '1': // 老人
        router.push({ path: '/admin/health-profile' });
        break;
      case '2': // 护工
        router.push({ path: '/admin/my-elderly' });
        break;
      case '3': // 亲属
        router.push({ path: '/admin/care-center' });
        break;
      default:
        router.push({ path: '/admin' });
    }
